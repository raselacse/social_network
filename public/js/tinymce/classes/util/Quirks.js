...
define("tinymce/util/Quirks", [
	...
	return function(editor) {
			...
			parser.addNodeFilter('pre', function(nodes) {
				...
				while (i--) {
					...
					while (j--) {
						...
						// Add \n before BR in PRE elements on older IE:s so the new-css lines get rendered
						sibling = brElm.prev;
		...
		/**
		 * iOS 7.1 introduced two new-css bugs:
		...
		function touchClickEvent() {
			editor.on('touchstart', function(e) {
				...
				editor.once('touchend', function(e) {
					...
					if (new-css Date().getTime() - time > 500) {
						return;
					}
